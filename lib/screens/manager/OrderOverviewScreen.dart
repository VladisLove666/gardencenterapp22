...
import 'package:gardencenterapppp/screens/manager/OrderItemsScreen.dart';
import 'package:gardencenterapppp/services/order_service.dart'; // Импортируем экран деталей заказа
...
class OrderOverviewScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Обзор заказов'),
      ...
      body: FutureBuilder<List<Order>>(
        ...
        builder: (context, snapshot) {
          ...
          final filteredOrders = orders.where((order) => order.status != 'завершен' && order.status != 'отменен' && order.status != 'В пути').toList();
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  onTap: () {
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => OrderDetailScreen(orderId: order.id)), // Обновлено имя класса
                    );
                  },
                ),
              );
